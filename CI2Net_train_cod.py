...
from models.CI2Net import CI2Net
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        if epoch > 110:
            torch.save(model.state_dict(), save_path + 'CI2Net_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'CI2Net_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'CI2Net_epoch_best.pth')
